...
print("继续 Phase 35 安装...")

# 1. repository.py - 审计账本
repository_content = '''"""Retention 审计账本：保存 Plan、确认、逐步 journal 和 hold。"""
...
with open("app/retention/repository.py", "w") as f:
    f.write(repository_content)
print("✓ 创建 app/retention/repository.py")

# 2. lock.py - 单主机 Sweep 互斥锁
lock_content = '''"""单主机 Sweep 文件锁。"""
from __future__ import annotations
import fcntl
...
with open("app/retention/lock.py", "w") as f:
    f.write(lock_content)
print("✓ 创建 app/retention/lock.py")

# 3. checkpoint_adapter.py
checkpoint_content = '''"""LangGraph Checkpoint 删除适配器。"""
from __future__ import annotations
...
with open("app/retention/checkpoint_adapter.py", "w") as f:
    f.write(checkpoint_content)
print("✓ 创建 app/retention/checkpoint_adapter.py")

print("已完成 Phase 35 部分文件创建！")
print("还需要实现：inventory.py, paths.py, service.py, factory.py")
print("以及修改现有文件...")